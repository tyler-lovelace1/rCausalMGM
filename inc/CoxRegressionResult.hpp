...
/**
 * Stores the various components of a Cox coxregression result so they can be passed
...
class CoxRegressionResult
{
  ...
  public:
    CoxRegressionResult() {}
    CoxRegressionResult(std::vector<std::string> regressorNames, int n, arma::vec b, arma::vec t,
			arma::vec p, arma::vec se, double alpha, double loglikelihood);
    // ~CoxregressionResult();
    ...
    friend std::ostream& operator<<(std::ostream& os, CoxRegressionResult& crr);
