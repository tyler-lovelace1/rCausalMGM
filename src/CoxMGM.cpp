...
arma::vec CoxMGM::coxGradHess(arma::mat& eta, arma::mat& grad, arma::mat& diagHess) {
    ...
    double HsumTheta, nEvents, sub, d, phi, theta_weight_sum, theta_weight2_sum, dSum, dSum2;
	...
	theta_weight2_sum = 0;
	int i = 0;
	for (int j = 0; j < HList[m].n_elem; j++) {
	    HsumTheta = 0;
	    nEvents = 0;
	    sub = 0;
	    dSum = 0;
	    ...
	    for (int k = 0; k < HList[m][j]; k++) {
		if (censMat(orderMat(i+k, m), m)) {
		    nEvents++;
		    HsumTheta += theta(orderMat(i+k, m), m);
		    loss[m] += eta(orderMat(i+k, m), m);
		}
		sub += theta(orderMat(i+k, m), m);
	    }

	    if (nEvents > 0) {
		if (sub - rs_sum[m] > 1e-5) {
		    if ((HList[m][j] + i) != n) {
			arma::uvec indices = orderMat(arma::span(i,n-1), m);
			arma::uvec col = { (uint) m };
			rs_sum[m] = arma::accu(theta.submat(indices, col));
			if (sub - rs_sum[m] > 1e-5) {
			    throw std::runtime_error("Error in CoxMGM coxGradHess, sub > rs_sum: " + std::to_string(sub) + " > " + std::to_string(rs_sum[m]));
			}
		    } else {
			sub = rs_sum[m];
		    }
	    ...
	    for (int k = 0; k < HList[m][j]; k++) {
		theta_weight(orderMat(i+k, m), m) = theta_weight_sum - censMat(orderMat(i+k, m), m) * dSum;
		theta_weight2(orderMat(i+k, m), m) = theta_weight2_sum - censMat(orderMat(i+k, m), m) * dSum2;
	    }
