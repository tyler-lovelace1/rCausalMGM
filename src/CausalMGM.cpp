...
CausalMGM::CausalMGM(EdgeListGraph& graph, arma::mat& x, arma::mat& y, std::vector<Node>& variables, std::vector<int>& l) : graph(graph), xDat(x), yDat(y)  {
    ...
    for (int i = 0; i < p+q; i++) {
	nodeIdxMap.insert(std::pair<Node,int>(variables[i], i));
    }
...
CausalMGM::CausalMGM(DataSet ds, EdgeListGraph& graph) : graph(graph) {
    ...
    for (int i = 0; i < p+q; i++) {
	nodeIdxMap.insert(std::pair<Node,int>(this->variables.at(i), i));
    }
...
void CausalMGM::initParameters() {
	...
	arma::mat((int) xDat.n_cols, lsum,              arma::fill::zeros),  // thetaT
...
// init a binary mask for the set of active parameters
void CausalMGM::initActiveSet() {
    lcumsum = std::vector<int>(l.size()+1);
    lcumsum[0] = 0;
    for(int i = 0; i < l.size(); i++){
        lcumsum[i+1] = lcumsum[i] + l[i];
    ...
    active = CausalMGMParams(
        arma::mat((int) xDat.n_cols, (int) xDat.n_cols, arma::fill::zeros),  // beta
        arma::vec((int) xDat.n_cols,                    arma::fill::ones),   // betad
        arma::mat(lsum,              (int) xDat.n_cols, arma::fill::zeros),  // theta
	arma::mat((int) xDat.n_cols, lsum,              arma::fill::zeros),  // thetaT
        arma::mat(lsum,              lsum,              arma::fill::zeros),  // phi
        arma::vec((int) xDat.n_cols,                    arma::fill::ones),   // alpha1
        arma::vec(lsum,                                 arma::fill::ones)    // alpha2
	);

    nParams += 2 * ((int) xDat.n_cols) + lsum - yDat.n_cols;
    // Rcpp::Rcout << "Beginning setting active set...\n";
    // Rcpp::Rcout << active << std::endl;

    for (int i = 0; i < q; i++) {
	active.alpha2(lcumsum[i]) = 0;
    }
	...
	// Rcpp::Rcout << edge.toString() << std::endl;
	// Rcpp::Rcout << "  ( " << i << ", " << j << " )\n";

	if (a.isContinuous()) {
	    if (b.isContinuous()) {
		nParams++;
		if (edge.isUndirected() || edge.isNondirected()) {
		    active.beta(i,j) = 1;
		    active.beta(j,i) = 1;
		} else if (edge.isBidirected()) {
		    throw std::runtime_error("CausalMGM parameter estimation is not yet implemented for PAGs");
		} else {
		    if (edge.getDistalEndpoint(a) == ENDPOINT_ARROW) {
			active.beta(j,i) = 1;
		    } else if (edge.getDistalEndpoint(b) == ENDPOINT_ARROW) {
			active.beta(i,j) = 1;
		    } else {
			throw std::runtime_error("Invalid edge type: " + edge.toString());
		    }
		}
		...
		if (edge.isUndirected() || edge.isNondirected()) {
		    active.theta.submat(lcumsum[j]+1, i, lcumsum[j+1]-1, i).fill(1);
		    active.thetaT.submat(i, lcumsum[j]+1, i, lcumsum[j+1]-1).fill(1);
		} else if (edge.isBidirected()) {
		    throw std::runtime_error("CausalMGM parameter estimation is not yet implemented for PAGs");
		} else {
		    if (edge.getDistalEndpoint(a) == ENDPOINT_ARROW) {
		...
		throw std::runtime_error("Invalid node type for node " + b.getName());
	    }
		...
		if (edge.isUndirected() || edge.isNondirected()) {
		    active.theta.submat(lcumsum[i]+1, j, lcumsum[i+1]-1, j).fill(1);
		    active.thetaT.submat(j, lcumsum[i]+1, j, lcumsum[i+1]-1).fill(1);
		} else if (edge.isBidirected()) {
		    throw std::runtime_error("CausalMGM parameter estimation is not yet implemented for PAGs");
		} else {
		    if (edge.getDistalEndpoint(a) == ENDPOINT_ARROW) {
		...
		if (edge.isUndirected() || edge.isNondirected()) {
		    active.phi.submat(lcumsum[i]+1, lcumsum[j]+1,
				      lcumsum[i+1]-1, lcumsum[j+1]-1).fill(1);
		    active.phi.submat(lcumsum[j]+1, lcumsum[i]+1,
				      lcumsum[j+1]-1, lcumsum[i+1]-1).fill(1);
		} else if (edge.isBidirected()) {
		    throw std::runtime_error("CausalMGM parameter estimation is not yet implemented for PAGs");
		} else {
		    if (edge.getDistalEndpoint(a) == ENDPOINT_ARROW) {
	    ...
	    throw std::runtime_error("Invalid node type for node " + a.getName());
	}
    }

    // Rcpp::Rcout << "Active set:\n" << active << std::endl;
    
}

// avoid underflow in log(sum(exp(x))) calculation
double CausalMGM::logsumexp(const arma::vec& x) {
...
// double CausalMGM::calcLambdaMax() {
//     lcumsum = std::vector<int>(l.size()+1);
//     lcumsum[0] = 0;
//     for(int i = 0; i < l.size(); i++){
//         lcumsum[i+1] = lcumsum[i] + l[i];
...
//     CausalMGMParams nullGrad(nullGrad1D, p, lsum);
 ...
 * @return value of g(X)
 */
double CausalMGM::smooth(arma::vec& parIn, arma::vec& gradOutVec) {
    CausalMGMParams par(parIn, p, lsum);

    // Rcpp::Rcout << "par: \n" << par << std::endl;

    CausalMGMParams gradOut;
    ...
    arma::mat wxProd = xDat * par.thetaT + dDat * par.phi;
    ...
    gradOut.beta = active.beta % ((active.beta % gradOut.beta).t() + active.beta % gradOut.beta);

    //gradalpha1=diag(betad)*sum(res,0)';
    gradOut.alpha1 = arma::sum(tempLoss, 0).t();

    //gradtheta=D'*(res);
    gradOut.theta = active.theta % (dDat.t() * tempLoss);
    gradOut.thetaT = active.thetaT % (dDat.t() * tempLoss).t();
    ...
    */
    ...
    gradOut.theta += active.theta % gradW.t();
    gradOut.thetaT += active.thetaT % gradW;

    //gradphi=D'*wxprod;
    gradOut.phi = dDat.t() * wxProd;

    //gradphi=tril(gradphi)'+triu(gradphi);
    gradOut.phi = active.phi % ((active.phi % gradOut.phi).t() + active.phi % gradOut.phi);
    ...
    */
    ...
    gradOut.thetaT /= (double) n;
    gradOut.phi /= (double) n;

    // gradOut.beta += lambda(0) * par.beta;
    // gradOut.theta += lambda(1) * par.theta;
    // gradOut.thetaT += lambda(1) * par.thetaT;
    // gradOut.phi += lambda(2) * par.phi;
    ...
    gradOutVec = gradOut.toMatrix1D() % active.toMatrix1D();

    return (sqloss + catloss) / ((double) n);
}

double CausalMGM::smoothValue(arma::vec& parIn) {
    CausalMGMParams par(parIn, p, lsum);
 ...
 * @return value of h(X)
 */
double CausalMGM::nonSmooth(double t, arma::vec& X, arma::vec& pX) {
    double nonSmooth = 0;
    ...
    for (Edge edge : graph.getEdges()) {
    	...
    	int i = nodeIdxMap[a];
    	int j = nodeIdxMap[b];
    	// double weight = 1; // weightMat(i,j);

    	if (a.isContinuous()) {
    	    if (b.isContinuous()) {
    		if (edge.isUndirected() || edge.isNondirected()) {
    		    par.beta(i,j) /= (1 + tlam(0));
    		    par.beta(j,i) /= (1 + tlam(0));
    		    betaNorms += 0.5 * std::pow(par.beta(i,j),2);
    		} else if (edge.isBidirected()) {
    		    throw std::runtime_error("CausalMGM parameter estimation is not yet implemented for PAGs");
    		} else {
    		    if (edge.getDistalEndpoint(a) == ENDPOINT_ARROW) {
    			par.beta(j,i) /= (1 + tlam(0));
    			betaNorms += 0.5 * std::pow(par.beta(j,i),2);
    		    } else if (edge.getDistalEndpoint(b) == ENDPOINT_ARROW) {
    			par.beta(i,j) /= (1 + tlam(0));
    			betaNorms += 0.5 * std::pow(par.beta(i,j),2);
    		    } else {
    		...
    		if (edge.isUndirected() || edge.isNondirected()) {
    		    par.theta.submat(lcumsum[j], i, lcumsum[j+1]-1, i) /= (1 + tlam(1));
    		    par.thetaT.submat(i, lcumsum[j], i, lcumsum[j+1]-1) /= (1 + tlam(1));

    		    double norm2 = arma::norm(
			par.theta.col(i).subvec(lcumsum[j],
						lcumsum[j+1]-1), 2);
    		    thetaNorms += 0.5 * norm2;
    		} else if (edge.isBidirected()) {
    		    throw std::runtime_error("CausalMGM parameter estimation is not yet implemented for PAGs");
    		} else {
    		    if (edge.getDistalEndpoint(a) == ENDPOINT_ARROW) {
    			...
    			thetaNorms += 0.5 * norm2;
    		        ...
    		        par.thetaT.submat(i, lcumsum[j], i, lcumsum[j+1]-1) /= (1 + tlam(1));
    			double norm2 = arma::norm(
			    par.thetaT.row(i).subvec(lcumsum[j],
						     lcumsum[j+1]-1),2);
    			thetaNorms += 0.5 * norm2;
    		    } else {
    			throw std::runtime_error("Invalid edge type: "+edge.toString());
    		...
    		if (edge.isUndirected() || edge.isNondirected()) {
    		    par.theta.submat(lcumsum[i], j, lcumsum[i+1]-1, j) /= (1 + tlam(1));
    		    par.thetaT.submat(j, lcumsum[i], j, lcumsum[i+1]-1) /= (1 + tlam(1));
    		    double norm2 = arma::norm(
			par.theta.col(j).subvec(lcumsum[i],
						lcumsum[i+1]-1),2);
    		    thetaNorms += 0.5 * norm2;
    		} else if (edge.isBidirected()) {
    		    throw std::runtime_error("CausalMGM parameter estimation is not yet implemented for PAGs");
    		    ...
    		    if (edge.getDistalEndpoint(a) == ENDPOINT_ARROW) {
    		        par.thetaT.submat(j, lcumsum[i], j, lcumsum[i+1]-1) /= (1 + tlam(1));
    		        double norm2 = arma::norm(
			    par.thetaT.row(j).subvec(lcumsum[i],
						     lcumsum[i+1]-1),2);
    			thetaNorms += 0.5 * norm2;
    		    } else if (edge.getDistalEndpoint(b) == ENDPOINT_ARROW) {
    		        par.theta.submat(lcumsum[i], j, lcumsum[i+1]-1, j) /= (1 + tlam(1));
    		...
    		if (edge.isUndirected() || edge.isNondirected()) {
    		    par.phi.submat(lcumsum[i], lcumsum[j],
				   lcumsum[i+1]-1, lcumsum[j+1]-1) /= (1 + tlam(2));
    		    par.phi.submat(lcumsum[j], lcumsum[i],
				   lcumsum[j+1]-1, lcumsum[i+1]-1) /= (1 + tlam(2));
    		    double normFro = arma::norm(
			par.phi.submat(lcumsum[i], lcumsum[j],
				       lcumsum[i+1]-1, lcumsum[j+1]-1), "fro");
    		    phiNorms += 0.5 * normFro;
    		} else if (edge.isBidirected()) {
    		    throw std::runtime_error("CausalMGM parameter estimation is not yet implemented for PAGs");
    		} else {
    		    if (edge.getDistalEndpoint(a) == ENDPOINT_ARROW) {
    			...
    			double normFro = arma::norm(
			    par.phi.submat(lcumsum[j], lcumsum[i],
					   lcumsum[j+1]-1, lcumsum[i+1]-1), "fro");
    			phiNorms += 0.5 * normFro;
    		    } else if (edge.getDistalEndpoint(b) == ENDPOINT_ARROW) {
    		        par.phi.submat(lcumsum[i], lcumsum[j],
    				       lcumsum[i+1]-1, lcumsum[j+1]-1) /= (1 + tlam(2));
 ...
 * @return value of h(X)
 */
double CausalMGM::nonSmoothValue(arma::vec& parIn) {
 ...
 * @return
 */
arma::vec CausalMGM::smoothGradient(arma::vec& parIn) {
    int n = xDat.n_rows;
    CausalMGMParams grad;
    ...
    //gradbeta=gradbeta-diag(diag(gradbeta)); % zero out diag
    //gradbeta=tril(gradbeta)'+triu(gradbeta);
    grad.beta = active.beta % ((active.beta % grad.beta).t() + active.beta % grad.beta);
    ...
    grad.theta = active.theta % (dDat.t() * negLoss);
    grad.thetaT = active.thetaT % (dDat.t() * negLoss).t();
    ...
    //gradtheta=gradtheta+gradw';
    // grad.theta += gradW.t();
    grad.theta += active.theta % gradW.t();
    grad.thetaT += active.thetaT % gradW;
    // Rcpp::Rcout << "grad.theta: \n" << grad.theta << std::endl;
    ...
    grad.phi = active.phi % ((active.phi % grad.phi).t() + active.phi % grad.phi);
    ...
    */
    ...
    grad.beta /= (double) n;
    grad.theta /= (double) n;
    grad.thetaT /= (double) n;
    grad.phi /= (double) n;

    // grad.beta += lambda(0) * par.beta;
    // grad.theta += lambda(1) * par.theta;
    // grad.thetaT += lambda(1) * par.thetaT;
    // grad.phi += lambda(2) * par.phi;
    ...
    return grad.toMatrix1D() % active.toMatrix1D();
}
 ...
 */
arma::vec CausalMGM::proximalOperator(double t, arma::vec& X) {
    if (t <= 0)
 ...
 */
void CausalMGM::learn(double epsilon, int iterLimit) {
    ...
    params = CausalMGMParams(newParams, p, lsum);
}
...
//  * @param iterLimit
//  */
// void CausalMGM::learnEdges(int iterLimit) {
...
//  * @param edgeChangeTol
//  */
// void CausalMGM::learnEdges(int iterLimit, int edgeChangeTol){
//     ProximalGradient pg(0.5, 0.9, true);
//     arma::vec curParams = params.toMatrix1D();
...
//  * @return
//  */
// arma::mat CausalMGM::adjMatFromCausalMGM() {
...
// /**
//  * Converts CausalMGM object to Graph object with edges if edge parameters are non-zero. Loses all edge param information
//  *
//  * @return
//  */
// EdgeListGraph CausalMGM::graphFromCausalMGM() {
 ...
 * @return
 */
CausalMGMParams CausalMGM::search() {
...
// std::vector<EdgeListGraph> CausalMGM::searchPath(std::vector<double> lambdas,
// 						 arma::vec& loglik,
// 						 arma::vec& nParams) {
//     auto start = std::chrono::high_resolution_clock::now();
...
// 	if (verbose) RcppThread::Rcout << "  Learning CausalMGM for lambda = " << lambdas[i] << "\n";
// 	std::vector<double> lambda = { lambdas[i], lambdas[i], lambdas[i] };
// 	setLambda(lambda);
// 	learn(1e-5, 500);
// 	pathGraphs.push_back(graphFromCausalMGM());
...
// std::vector<EdgeListGraph> CausalMGM::searchPathCV(std::vector<double> lambdas,
// 						   int nfolds,
// 						   arma::uvec& foldid,
// 						   arma::mat& loglik) {
//     auto start = std::chrono::high_resolution_clock::now();
//     std::vector<EdgeListGraph> cvGraphs;
//     // std::vector<CausalMGM> trainCausalMGMs;
//     // std::vector<CausalMGM> testCausalMGMs;
//     std::sort(lambdas.begin(), lambdas.end(), std::greater<double>());
...
// 	CausalMGM trainCausalMGM(arma::mat(xDat.rows(trainIdxs)),
// 				 arma::mat(yDat.rows(trainIdxs)),
// 				 variables, l, lambda);

// 	trainCausalMGM.setVerbose(false);
	
//         CausalMGM testCausalMGM(arma::mat(xDat.rows(testIdxs)),
// 				arma::mat(yDat.rows(testIdxs)),
// 				variables, l, lambda);
	
// 	for (int i = 0; i < lambdas.size(); i++) {
// 	    if (verbose) {
// 		RcppThread::Rcout << "  Fold " << k << ": Learning CausalMGM for lambda = "
// 				  << lambdas[i] << "\r";
// 	    }
...
// 	    trainCausalMGM.setLambda(lambda);
	    
// 	    // Rcpp::Rcout << "Training...\n";
	    
// 	    trainCausalMGM.learn(1e-5, 500);
// 	    // pathGraphs.push_back(graphFromCausalMGM());

// 	    arma::vec par(trainCausalMGM.getParams().toMatrix1D());

// 	    // Rcpp::Rcout << "Evaluating on test data...\n";
// 	    loglik(i,k-1) = testCausalMGM.smoothValue(par);
// 	    // nParams(i) = arma::accu(par!=0);
...
//     if (verbose) RcppThread::Rcout << "alpha (1 SE) = " << lambdas[seIdx] << "\n";

//     setLambda(lambda);
//     learn(1e-5, 500);

//     cvGraphs.push_back(graphFromCausalMGM());

//     lambda = { lambdas[minIdx], lambdas[minIdx], lambdas[minIdx] };

//     if (verbose) RcppThread::Rcout << "alpha (min) = " << lambdas[minIdx] << "\n";
