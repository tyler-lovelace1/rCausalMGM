...
bool AnnealBoss::OrderGraph::operator==(const OrderGraph& rhs) const {
    if (score == rhs.score) {
...
std::list<Node> AnnealBoss::initializeRandom() {
...
AnnealBoss::AnnealBoss(Score *scorer, int threads) : taskQueue(MAX_QUEUE_SIZE) {
...
void AnnealBoss::setNumStarts(int numStarts) {
    if (numStarts < 1)
...
double AnnealBoss::update(OrderGraph& tau) {
...
double AnnealBoss::updateParallel(OrderGraph& tau) {
...
// AnnealBoss::OrderGraph AnnealBoss::bestMove(Node n, OrderGraph tau) {
...
AnnealBoss::OrderGraph AnnealBoss::bestMove(Node n, OrderGraph tau) {
    ...
    double paccept = 1.0 / (1.0 + std::exp((minBic - tau.bic)/(2*temperature)));

    if (R::runif(0,1) > paccept) return tau;
...
bool AnnealBoss::bossForward() {
    ...
    bool improved = std::abs(tau.bic - pi.bic) > 1e-6;
...
std::set<Node> AnnealBoss::getNaXY(const Node& x, const Node& y, EdgeListGraph& graph) {
    std::set<Node> naXY;
...
bool AnnealBoss::isClique(const std::set<Node>& nodes, EdgeListGraph& graph) {
...
bool AnnealBoss::validDelete(EdgeListGraph& graph, DeleteTask& task) {
...
void AnnealBoss::deleteProducer(EdgeListGraph& graph) {
    // int idx = 0;
...
void AnnealBoss::deleteConsumer(EdgeListGraph& graph) {
    while(true) {
	...
	double paccept = 1.0 / (1.0 + std::exp(scoreChange/(2*temperature)));


	if (R::runif(0,1) <= paccept /*scoreChange < -1e-6*/) {
	    std::lock_guard<std::mutex> deleteLock(deleteMutex);
	    RcppThread::Rcout << "      Inner Deletion: " << task.x
...
bool AnnealBoss::bestDelete(EdgeListGraph& graph) {
...
bool AnnealBoss::bossBES() {
    ...
    return (std::abs(piGraph.getScore() - oldBic) > 1e-6) || (std::abs(pi.bic - oldBic) > 1e-6); // (piGraph.getScore() - oldBic < -1e-6) || (pi.bic - oldBic < -1e-6); // std::abs(pi.bic - oldBic) > 1e-6; //  || (pi.score - piGraph.getNumEdges() < -0.5)
}


EdgeListGraph AnnealBoss::search() {
