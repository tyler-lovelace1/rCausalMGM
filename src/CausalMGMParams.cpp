...
CausalMGMParams::CausalMGMParams(const arma::mat& beta, const arma::vec& betad, const arma::mat& theta, const arma::mat& thetaT, const arma::mat& phi, const arma::vec& alpha1, const arma::vec& alpha2) :
  beta(beta), betad(betad), theta(theta), thetaT(thetaT), phi(phi), alpha1(alpha1), alpha2(alpha2) {
    // this->beta = beta;
    // this->betad = betad;
    // this->theta = theta;
    // this->thetaT = thetaT;
...
// // Copy constructor
// CausalMGMParams::CausalMGMParams(CausalMGMParams& parIn) {
...
CausalMGMParams::CausalMGMParams(arma::vec& vec, int p, int ltot) {
    std::vector<int> lens{p*p, p, p*ltot, p*ltot, ltot*ltot, p, ltot};
    ...
    if (vec.n_elem != lenSums[6])
        throw std::invalid_argument("Param vector dimension doesn't match: Found " + std::to_string(vec.n_elem) + " need " + std::to_string(lenSums[6]));
    ...
    thetaT = arma::reshape(vec.subvec(lenSums[2], lenSums[3]-1), p, ltot);
    phi = arma::reshape(vec.subvec(lenSums[3], lenSums[4]-1), ltot, ltot);
    alpha1 = arma::vec(vec.subvec(lenSums[4], lenSums[5]-1));
    alpha2 = arma::vec(vec.subvec(lenSums[5], lenSums[6]-1));
}

/**
 * Copy all params into a single vector
 */
arma::vec CausalMGMParams::toMatrix1D() {
    int p = alpha1.n_elem;
    ...
    arma::vec outVec(p*p + p + p*ltot + p*ltot + ltot*ltot + p + ltot);
    ...
    outVec.subvec(lenSums[2], arma::size(thetaT.as_col())) = thetaT.as_col();
    outVec.subvec(lenSums[3], arma::size(phi.as_col())) = phi.as_col();
    outVec.subvec(lenSums[4], arma::size(alpha1)) = alpha1;
    outVec.subvec(lenSums[5], arma::size(alpha2)) = alpha2;
    
    return outVec;
}

Rcpp::List CausalMGMParams::toList() const {
    int p = beta.n_cols;
    int ltot = phi.n_cols;
    ...
    Rcpp::NumericMatrix _thetaT = Rcpp::wrap(thetaT);
    colnames(_thetaT) = _discNames;
    rownames(_thetaT) = _contNames;
...
std::ostream& operator<<(std::ostream& os, CausalMGMParams& params) {
    os << "alpha1:\n" << params.alpha1.t() << "\n";
    os << "alpha2:\n" << params.alpha2.t() << "\n";
    os << "beta:\n"   << params.beta       << "\n";
    os << "betad:\n"  << params.betad.t()  << "\n";
