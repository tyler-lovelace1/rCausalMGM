#include "SepsetProducerMajority.hpp"
#include "GraphUtils.hpp"

void SepsetProducerMajority::setDepth(int depth) {
    if (depth < -1)
...
void SepsetProducerMajority::producer() {
...
void SepsetProducerMajority::consumer() {
    while(true) {
...
void SepsetProducerMajority::fillMap() {
...
std::vector<Node>  SepsetProducerMajority::getSepset(const Node& a, const Node& b) {
...
bool SepsetProducerMajority::isCollider(const Node& i, const Node& j, const Node& k) {
    ...
    return count.second / (double) std::max(count.first + count.second, 1) > 0.5;
}

bool SepsetProducerMajority::isNoncollider(const Node& i, const Node& j, const Node& k) {
    if (sepsetCount.count(Triple(i,j,k)) == 0) {
	std::vector<Node> sepset = getSepset(i,k);
	return (std::find(sepset.begin(), sepset.end(), j) != sepset.end());
    }
